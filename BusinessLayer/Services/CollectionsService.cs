...
namespace BusinessLayer.Services
{
    public class CollectionsService : ICollectionsService
    {
        ...
        public CollectionsService(ICollectionsRepository collectionsRepository, OwnedGamesService ownedGamesService)
        {
        ...
        public List<Collection> GetAllCollections(int userId)
        {
            try
            {
                ...
                Debug.WriteLine("Service: Retrieved " + (collections != null ? collections.Count.ToString() : "0") + " collections from repository");
                return collections;
            }
        ...
        public Collection GetCollectionById(int collectionId, int userId)
        {
            try
            {
                Debug.WriteLine("Service: Getting collection " + collectionId);
                var collection = _collectionsRepository.GetCollectionById(collectionId, userId);
                if (collection == null)
                {
                    Debug.WriteLine("Service: No collection found with ID " + collectionId);
                    return null;
                }

                // Load games for the collection.
                collection.Games = _collectionsRepository.GetGamesInCollection(collectionId, userId);
                Debug.WriteLine("Service: Successfully retrieved collection " + collectionId + " with " + (collection.Games != null ? collection.Games.Count.ToString() : "0") + " games");
                return collection;
            }
            catch (RepositoryException ex)
            {
        ...
        public List<OwnedGame> GetGamesInCollection(int collectionId)
        {
            try
            {
                ...
                Debug.WriteLine("Service: Retrieved " + (games != null ? games.Count.ToString() : "0") + " games from repository");
                return games;
            }
            catch (RepositoryException ex)
            {
        ...
        public void RemoveGameFromCollection(int collectionId, int gameId)
        {
            try
            {
                Debug.WriteLine("Service: Removing game " + gameId + " from collection " + collectionId);
                _collectionsRepository.RemoveGameFromCollection(collectionId, gameId);
                Debug.WriteLine("Service: Successfully removed game " + gameId + " from collection " + collectionId);
            }
            catch (RepositoryException ex)
            {
                Debug.WriteLine("Service: Repository error: " + ex.Message);
        ...
        public List<Collection> GetPublicCollectionsForUser(int userId)
        {
            try
            {
                ...
                Debug.WriteLine("Service: Retrieved " + (collections != null ? collections.Count.ToString() : "0") + " public collections from repository");
                return collections;
            }
            catch (RepositoryException ex)
            {
